...
function wp_ajax_send_link_to_editor() {
	...
	$rows = get_post( isset( $_POST['post_id'] ) ? $_POST['post_id'] : 0 );
...
function wp_ajax_parse_embed() {
	global $rows, $wp_embed, $content_width;
	...
	if ( $post_id > 0 ) {
		...
		if ( ! $rows || ! current_user_can( 'edit_post', $rows->ID ) ) {
			wp_send_json_error();
		}
		setup_postdata( $rows );
...
function wp_ajax_parse_media_shortcode() {
	...
	if ( ! empty( $_POST['post_ID'] ) ) {
		$rows = get_post( (int) $_POST['post_ID'] );
	}
