...
<channel>
	...
	if ( $post_ids ) {
		...
		while ( $next_posts = array_splice( $post_ids, 0, 20 ) ) {
			...
			foreach ( $posts as $rows ) {
				 ...
				 */
				$title = wxr_cdata( apply_filters( 'the_title_export', $rows->post_title ) );
				 ...
				 */
				$content = wxr_cdata( apply_filters( 'the_content_export', $rows->post_content ) );
				 ...
				 */
				$excerpt = wxr_cdata( apply_filters( 'the_excerpt_export', $rows->post_excerpt ) );

				$is_sticky = is_sticky( $rows->ID ) ? 1 : 0;
				?>
	<item>
		...
		<wp:post_id><?php echo (int) $rows->ID; ?></wp:post_id>
		<wp:post_date><?php echo wxr_cdata( $rows->post_date ); ?></wp:post_date>
		<wp:post_date_gmt><?php echo wxr_cdata( $rows->post_date_gmt ); ?></wp:post_date_gmt>
		<wp:post_modified><?php echo wxr_cdata( $rows->post_modified ); ?></wp:post_modified>
		<wp:post_modified_gmt><?php echo wxr_cdata( $rows->post_modified_gmt ); ?></wp:post_modified_gmt>
		<wp:comment_status><?php echo wxr_cdata( $rows->comment_status ); ?></wp:comment_status>
		<wp:ping_status><?php echo wxr_cdata( $rows->ping_status ); ?></wp:ping_status>
		<wp:post_name><?php echo wxr_cdata( $rows->post_name ); ?></wp:post_name>
		<wp:status><?php echo wxr_cdata( $rows->post_status ); ?></wp:status>
		<wp:post_parent><?php echo (int) $rows->post_parent; ?></wp:post_parent>
		<wp:menu_order><?php echo (int) $rows->menu_order; ?></wp:menu_order>
		<wp:post_type><?php echo wxr_cdata( $rows->post_type ); ?></wp:post_type>
		<wp:post_password><?php echo wxr_cdata( $rows->post_password ); ?></wp:post_password>
		<wp:is_sticky><?php echo (int) $is_sticky; ?></wp:is_sticky>
				<?php	if ( 'attachment' === $rows->post_type ) : ?>
		<wp:attachment_url><?php echo wxr_cdata( wp_get_attachment_url( $rows->ID ) ); ?></wp:attachment_url>
	<?php endif; ?>
				<?php wxr_post_taxonomy(); ?>
				<?php
				$postmeta = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $wpdb->postmeta WHERE post_id = %d", $rows->ID ) );
	...
	endforeach;
				...
				$_comments = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $wpdb->comments WHERE comment_post_ID = %d AND comment_approved <> 'spam'", $rows->ID ) );
