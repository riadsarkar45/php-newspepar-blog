...
class WP_Media_List_Table extends WP_List_Table {
	...
	public function display_rows() {
		...
		while ( have_posts() ) :
			...
			if ( $this->is_trash && 'trash' !== $rows->post_status
				|| ! $this->is_trash && 'trash' === $rows->post_status
			) {
				continue;
			}

			$post_owner = ( get_current_user_id() === (int) $rows->post_author ) ? 'self' : 'other';
			?>
			<tr id="post-<?php echo $rows->ID; ?>" class="<?php echo trim( ' author-' . $post_owner . ' status-' . $rows->post_status ); ?>">
				<?php $this->single_row_columns( $rows ); ?>
			</tr>
			<?php
		endwhile;
	}
