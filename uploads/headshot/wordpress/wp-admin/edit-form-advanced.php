 ...
 * @global WP_Post      $rows             Global post object.
 */
global $post_type, $post_type_object, $rows;
...
if ( (int) get_option( 'page_for_posts' ) === $rows->ID && empty( $rows->post_content ) ) {
	add_action( 'edit_form_after_title', '_wp_posts_page_notice' );
	remove_post_type_support( $post_type, 'editor' );
}

$thumbnail_support = current_theme_supports( 'post-thumbnails', $post_type ) && post_type_supports( $post_type, 'thumbnail' );
if ( ! $thumbnail_support && 'attachment' === $post_type && $rows->post_mime_type ) {
	if ( wp_attachment_is( 'audio', $rows ) ) {
		$thumbnail_support = post_type_supports( 'attachment:audio', 'thumbnail' ) || current_theme_supports( 'post-thumbnails', 'attachment:audio' );
	} elseif ( wp_attachment_is( 'video', $rows ) ) {
...
if ( $thumbnail_support ) {
	add_thickbox();
	wp_enqueue_media( array( 'post' => $rows->ID ) );
}
 ...
 */
$permalink = get_permalink( $rows->ID );
if ( ! $permalink ) {
	$permalink = '';
}
...
$preview_url = get_preview_post_link( $rows );
...
$scheduled_date = sprintf(
	...
	date_i18n( _x( 'M j, Y', 'publish box date format' ), strtotime( $rows->post_date ) ),
	/* translators: Publish box time format, see https://www.php.net/manual/datetime.format.php */
	date_i18n( _x( 'H:i', 'publish box time format' ), strtotime( $rows->post_date ) )
);
...
$notice     = false;
$form_extra = '';
if ( 'auto-draft' === $rows->post_status ) {
	if ( 'edit' === $action ) {
		$rows->post_title = '';
	}
	$autosave    = false;
	...
	$autosave = wp_get_post_autosave( $rows->ID );
}

$form_action  = 'editpost';
$nonce_action = 'update-post_' . $rows->ID;
$form_extra  .= "<input type='hidden' id='post_ID' name='post_ID' value='" . esc_attr( $rows->ID ) . "' />";

// Detect if there exists an autosave newer than the post and if that autosave is different than the post.
if ( $autosave && mysql2date( 'U', $autosave->post_modified_gmt, false ) > mysql2date( 'U', $rows->post_modified_gmt, false ) ) {
	foreach ( _wp_post_revision_fields( $rows ) as $autosave_field => $_autosave_field ) {
		if ( normalize_whitespace( $autosave->$autosave_field ) !== normalize_whitespace( $rows->$autosave_field ) ) {
			$notice = sprintf(
...
register_and_do_post_meta_boxes( $rows );
 ...
 */
do_action( 'post_edit_form_tag', $rows );
...
<input type="hidden" id="post_author" name="post_author" value="<?php echo esc_attr( $rows->post_author ); ?>" />
<input type="hidden" id="post_type" name="post_type" value="<?php echo esc_attr( $post_type ); ?>" />
<input type="hidden" id="original_post_status" name="original_post_status" value="<?php echo esc_attr( $rows->post_status ); ?>" />
<input type="hidden" id="referredby" name="referredby" value="<?php echo $referer ? esc_url( $referer ) : ''; ?>" />
<?php if ( ! empty( $active_post_lock ) ) { ?>
<input type="hidden" id="active_post_lock" value="<?php echo esc_attr( implode( ':', $active_post_lock ) ); ?>" />
	<?php
}
if ( 'draft' !== get_post_status( $rows ) ) {
...
<div id="titlewrap">
	 ...
	 * @param WP_Post $post Post object.
	 */
	$title_placeholder = apply_filters( 'enter_title_here', __( 'Add title' ), $rows );
	?>
	<label class="screen-reader-text" id="title-prompt-text" for="title"><?php echo $title_placeholder; ?></label>
	<input type="text" name="post_title" size="30" value="<?php echo esc_attr( $rows->post_title ); ?>" id="title" spellcheck="true" autocomplete="off" />
</div>
	<?php
	/**
	 ...
	 * @param WP_Post $post Post object.
	 */
	do_action( 'edit_form_before_permalink', $rows );
	?>
<div class="inside">
	<?php
	if ( $viewable ) :
		$sample_permalink_html = $post_type_object->public ? get_sample_permalink_html( $rows->ID ) : '';

		// As of 4.4, the Get Shortlink button is hidden by default.
		if ( has_filter( 'pre_get_shortlink' ) || has_filter( 'get_shortlink' ) ) {
			$shortlink = wp_get_shortlink( $rows->ID, 'post' );

			if ( ! empty( $shortlink ) && $shortlink !== $permalink && home_url( '?page_id=' . $rows->ID ) !== $permalink ) {
		...
		if ( $post_type_object->public
			&& ! ( 'pending' === get_post_status( $rows ) && ! current_user_can( $post_type_object->cap->publish_posts ) )
		) {
			$has_sample_permalink = $sample_permalink_html && 'auto-draft' !== $rows->post_status;
			?>
 ...
 * @param WP_Post $post Post object.
 */
do_action( 'edit_form_after_title', $rows );
...
<table id="post-status-info"><tbody><tr>
	...
	if ( 'auto-draft' !== $rows->post_status ) {
		...
		$last_user = get_userdata( get_post_meta( $rows->ID, '_edit_last', true ) );
		if ( $last_user ) {
			/* translators: 1: Name of most recent post author, 2: Post edited date, 3: Post edited time. */
			printf( __( 'Last edited by %1$s on %2$s at %3$s' ), esc_html( $last_user->display_name ), mysql2date( __( 'F j, Y' ), $rows->post_modified ), mysql2date( __( 'g:i a' ), $rows->post_modified ) );
		} else {
			/* translators: 1: Post edited date, 2: Post edited time. */
			printf( __( 'Last edited on %1$s at %2$s' ), mysql2date( __( 'F j, Y' ), $rows->post_modified ), mysql2date( __( 'g:i a' ), $rows->post_modified ) );
		}
		echo '</span>';
	}
	?>
 ...
 * @param WP_Post $post Post object.
 */
do_action( 'edit_form_after_editor', $rows );
?>
...
if ( 'page' === $post_type ) {
	 ...
	 * @param WP_Post $post Post object.
	 */
	do_action( 'submitpost_box', $rows );
}


do_meta_boxes( $post_type, 'side', $rows );
...
do_meta_boxes( null, 'normal', $rows );
...
if ( 'page' === $post_type ) {
	 ...
	 * @param WP_Post $post Post object.
	 */
	do_action( 'edit_form_advanced', $rows );
}


do_meta_boxes( null, 'advanced', $rows );
 ...
 * @param WP_Post $post Post object.
 */
do_action( 'dbx_post_sidebar', $rows );
...
<?php if ( ! wp_is_mobile() && post_type_supports( $post_type, 'title' ) && '' === $rows->post_title ) : ?>
