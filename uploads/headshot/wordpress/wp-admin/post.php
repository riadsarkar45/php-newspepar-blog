...
if ( isset( $_POST['post_type'] ) && $rows && $post_type !== $_POST['post_type'] ) {
...
switch ( $action ) {
	case 'post-quickdraft-save':
		...
		$rows = get_post( $_REQUEST['post_ID'] );
		check_admin_referer( 'add-' . $rows->post_type );

		$_POST['comment_status'] = get_default_comment_status( $rows->post_type );
		$_POST['ping_status']    = get_default_comment_status( $rows->post_type, 'pingback' );
	...
	case 'edit':
		 ...
		 */
		if ( true === apply_filters( 'replace_editor', false, $rows ) ) {
			break;
		}

		if ( use_block_editor_for_post( $rows ) ) {
			require ABSPATH . 'wp-admin/edit-form-blocks.php';
			break;
		}

		if ( ! wp_check_post_lock( $rows->ID ) ) {
		...
		$rows = get_post( $post_id, OBJECT, 'edit' );
