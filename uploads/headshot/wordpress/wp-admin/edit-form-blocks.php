 ...
 */
global $post_type, $post_type_object, $rows, $title, $wp_meta_boxes;

$block_editor_context = new WP_Block_Editor_Context( array( 'post' => $rows ) );
...
$rest_path = rest_get_route_for_post( $rows );
...
wp_add_inline_script(
	'wp-blocks',
	sprintf( 'wp.blocks.setCategories( %s );', wp_json_encode( get_block_categories( $rows ) ) ),
	'after'
);
...
if ( 'auto-draft' === $rows->post_status ) {
	...
	if ( post_type_supports( $rows->post_type, 'title' ) ) {
		$initial_edits['title'] = $rows->post_title;
	}

	if ( post_type_supports( $rows->post_type, 'editor' ) ) {
		$initial_edits['content'] = $rows->post_content;
	}

	if ( post_type_supports( $rows->post_type, 'excerpt' ) ) {
		$initial_edits['excerpt'] = $rows->post_excerpt;
	}
}
 ...
 */
$available_templates = wp_get_theme()->get_page_templates( get_post( $rows->ID ) );
...
// Lock settings.
$user_id = wp_check_post_lock( $rows->ID );
if ( $user_id ) {
	$locked = false;

	/** This filter is documented in wp-admin/includes/post.php */
	if ( apply_filters( 'show_post_locked_dialog', true, $rows, $user_id ) ) {
		$locked = true;
	}
	...
	// Lock the post.
	$active_post_lock = wp_set_post_lock( $rows->ID );
	if ( $active_post_lock ) {
 ...
 */
$body_placeholder = apply_filters( 'write_your_story', __( 'Type / to choose a block' ), $rows );

$editor_settings = array(
	'availableTemplates'   => $available_templates,
	'disablePostFormats'   => ! current_theme_supports( 'post-formats' ),
	/** This filter is documented in wp-admin/edit-form-advanced.php */
	'titlePlaceholder'     => apply_filters( 'enter_title_here', __( 'Add title' ), $rows ),
	'bodyPlaceholder'      => $body_placeholder,
	'autosaveInterval'     => AUTOSAVE_INTERVAL,
	'richEditingEnabled'   => user_can_richedit(),
	'postLock'             => $lock_details,
	'postLockUtils'        => array(
		'nonce'       => wp_create_nonce( 'lock-post_' . $rows->ID ),
		'unlockNonce' => wp_create_nonce( 'update-post_' . $rows->ID ),
...
if ( $autosave ) {
	if ( mysql2date( 'U', $autosave->post_modified_gmt, false ) > mysql2date( 'U', $rows->post_modified_gmt, false ) ) {
...
if ( $is_new_post && ! isset( $editor_settings['template'] ) && 'post' === $rows->post_type ) {
	$post_format = get_post_format( $rows );
...
if ( (int) get_option( 'page_for_posts' ) === $rows->ID ) {
		...
		<div class="notice notice-error notice-alt">
			<p>
				<?php
					 ...
					 */
					echo apply_filters( 'block_editor_no_javascript_message', $message, $rows );
					?>
			</p>
		</div>
	</div>
