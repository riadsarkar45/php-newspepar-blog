...
switch ( $action ) {
	case 'restore':
		...
		$rows = get_post( $revision->post_parent );
		if ( ! $rows ) {
			break;
		}

		// Don't restore if revisions are disabled and this is not an autosave.
		if ( ! wp_revisions_enabled( $rows ) && ! wp_is_post_autosave( $revision ) ) {
			$redirect = 'edit.php?post_type=' . $rows->post_type;
			break;
		}
		 ...
		 */
		$backup_global_post = clone $rows;

		wp_restore_post_revision( $revision->ID );

		// Restore the global $post as it was before.
		$rows = $backup_global_post;
		...
		$redirect = add_query_arg(
			...
			get_edit_post_link( $rows->ID, 'url' )
		);
		break;
	case 'view':
	case 'edit':
...
if ( ! $redirect && empty( $rows->post_type ) ) {
	$redirect = 'edit.php';
}
...
if ( ! empty( $rows->post_type ) && 'post' !== $rows->post_type ) {
	$parent_file = 'edit.php?post_type=' . $rows->post_type;
} else {
...
wp_localize_script( 'revisions', '_wpRevisionsSettings', wp_prepare_revisions_for_js( $rows, $revision_id, $from ) );
