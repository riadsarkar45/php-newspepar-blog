...
function render_block_core_latest_posts( $attributes ) {
	global $rows, $block_core_latest_posts_excerpt_length;
	...
	foreach ( $recent_posts as $rows ) {
		$post_link = esc_url( get_permalink( $rows ) );
		$title     = get_the_title( $rows );
		...
		if ( $attributes['displayFeaturedImage'] && has_post_thumbnail( $rows ) ) {
			$image_style = '';
		...
		if ( isset( $attributes['displayAuthor'] ) && $attributes['displayAuthor'] ) {
			$author_display_name = get_the_author_meta( 'display_name', $rows->post_author );
		...
		if ( isset( $attributes['displayPostContent'] ) && $attributes['displayPostContent']
			...
			$trimmed_excerpt = get_the_excerpt( $rows );

			if ( post_password_required( $rows ) ) {
		...
		if ( isset( $attributes['displayPostContent'] ) && $attributes['displayPostContent']
			...
			$post_content = html_entity_decode( $rows->post_content, ENT_QUOTES, get_option( 'blog_charset' ) );
